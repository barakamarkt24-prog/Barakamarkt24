...
export const WishlistScreen: React.FC = () => {
  const { wishlist, addToCart, navigateTo, showToast, currentUser, dir, t } = useApp();
  ...
  const handleAddAllToCart = () => {
    ...
    if (added > 0) {
      showToast(t('cart.itemCount', { count: added }));
    } else {
      showToast(t('wishlist.emptyTitle'));
    }
  };
  ...
  if (!currentUser) {
    return (
      <div className="p-4 space-y-4 pb-24 max-w-lg mx-auto" dir={dir}>
        <div className="py-12 text-center space-y-4 bg-white rounded-3xl p-6 border border-stone-200/80 shadow-2xs">
          ...
          <div className="space-y-1.5">
            <h2 className="font-black text-base text-stone-900">{t('wishlist.title')}</h2>
    ...
    <div className="p-4 space-y-4 pb-24 max-w-3xl mx-auto" dir={dir}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-stone-500 font-medium">{t('wishlist.savedItems', { count: wishlist.length })}</p>
        </div>
      ...
      {wishlist.length === 0 ? (
        <div className="py-16 text-center space-y-3 bg-white rounded-3xl p-6 border border-stone-200/80 shadow-2xs my-4">
          ...
          <h3 className="font-bold text-sm text-stone-900">{t('wishlist.emptyTitle')}</h3>
          <p className="text-xs text-stone-500 max-w-xs mx-auto leading-relaxed">
